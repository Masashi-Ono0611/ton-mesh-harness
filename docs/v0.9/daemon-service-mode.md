...
```bash
ton-mesh-harness service list            # installed seeds + site gateways + running state
ton-mesh-harness service stop <bag_id>   # stop + remove a bag-seeder unit (keep the db)
ton-mesh-harness service stop <bag_id> --purge   # also remove the seed db
ton-mesh-harness service stop-site <domain>      # stop + remove a site-gateway unit
```
...
installed as a parallel unit (`ton-mesh-site.<domain>` /
`ton-mesh-site-<domain>.service`, namespace `~/.ton-mesh/sites/<domain>/`).
...
`mesh_deploy` accepts `daemon_mode: "service"` (the OS owns the
