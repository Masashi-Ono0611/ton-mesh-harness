...
**Status:** v0.8.0-rc8 (2026-05-12) — full v0.8 feature ship + 6 rounds of Codex multi-model review (final round: 0 findings). MCP server (3 tools: `sovereign_check_env` / `sovereign_deploy` / `sovereign_status`), SDK external entry (`import { deploy } from 'ton-sovereign-deploy'`), NFT-delegated agentic signing (optional `@ton/mcp` peer), structured stderr logging (`DEBUG=sovereign:*`), tarball-install smoke in CI. Across 6 review rounds: 1 BLOCKER + 8 MAJORs + 1 MINOR + 4 NITs resolved. GA pending V3 (Claude Code MCP → testnet E2E) and V4 (agency-transfer red-team test).
