...
import { getBinaryName, getPlatformKey } from './platform'
...
const STORAGE_DAEMON_HASHES: Record<string, string> = {
...
const STORAGE_DAEMON_CLI_HASHES: Record<string, string> = {
...
export function ensureBinaries(useTestnet = false, opts: EnsureBinariesOptions = {}): void {
  ...
  if (needsBinaries) {
    ...
    verifyFileSha256({
      ...
      expected: STORAGE_DAEMON_HASHES[platformKey],
    })
    ...
    verifyFileSha256({
      ...
      platformKey,
      filePath: paths.cli,
      expected: STORAGE_DAEMON_CLI_HASHES[platformKey],
    })
